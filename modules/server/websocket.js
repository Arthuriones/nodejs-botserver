module.exports = async ({ config, utils, state }) => {
    ...
    const samePort = config.WS_PORT === config.HTTP_PORT;

    const wssOptions = samePort
        ? { server: state.httpServer, maxPayload: config.WS_MAX_PAYLOAD }
        : { port: config.WS_PORT, maxPayload: config.WS_MAX_PAYLOAD };

    const server = await new Promise((resolve, reject) => {
        const wss = new WebSocket.Server(wssOptions);

        if (samePort) {
            utils.log.success(`WebSocket attached to HTTP server on port ${config.HTTP_PORT}`);
